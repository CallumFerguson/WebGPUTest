...
@group(1) @binding(1) var environmentIrradianceCubeMapTexture: texture_cube<f32>;
@group(1) @binding(2) var environmentPrefilterCubeMapTexture: texture_cube<f32>;
@group(1) @binding(3) var brdfLUT: texture_2d<f32>;

@group(2) @binding(0) var<storage, read> bodies: array<Body>;
...
struct VertexOutput {
    ...
    @location(7) albedo: vec3f,
}

@vertex
fn vert(i: VertexInput) -> VertexOutput {
    let body = bodies[i.instanceIndex];

    var o: VertexOutput;

    let worldPosition = vec4(i.position * 0.5 * body.radius * 2 + body.position, 1);

    o.fragPosition = cameraData.projection * cameraData.view * worldPosition;
